...
import { getLayerStream } from './docker.js';
...
async function checkAllBatchesCompleted(env, masterTaskId) {
    ...
    // Docker layer 批次完成检查
    const allLayerBatchesDone = (master.completedLayerBatches?.length || 0) === (master.totalLayerBatches || 0);
    
    // 对于 GitHub 任务，totalAssetBatches 存在；对于 Docker 任务，totalLayerBatches 存在
    const allBatchesDone = allFileBatchesDone && (allAssetBatchesDone || allLayerBatchesDone);
    
    if (allBatchesDone) {
        const finalStatus = (master.failedFiles?.length > 0 || master.failedAssets?.length > 0 || master.failedLayers?.length > 0) 
...
// ==================== 新增：处理 Docker layer 任务 ====================
export async function processDockerLayer(layerTask, env) {
    const { masterTaskId, bucketId, owner, repo, tag, digest, size, imageName, batchIndex } = layerTask;
    ...
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
        try {
            // 获取 layer 流
            const stream = await getLayerStream(env, logMessages, imageName, digest);
            
            // 上传到 B2，路径如：owner/repo/tag/layers/digest_sha256
            const safeDigest = digest.replace(/[^a-zA-Z0-9]/g, '_'); // 替换特殊字符
            const b2Path = `${owner}/${repo}/${tag}/layers/${safeDigest}`;
            await uploadFile(client, bucket, b2Path, stream, size);
            
            // 更新主任务进度
            const masterKey = `master:${masterTaskId}`;
            const master = await env.B2_KV.get(masterKey, 'json') || {};
            const processedLayers = (master.processedLayers || 0) + 1;
            const completedLayerBatches = master.completedLayerBatches || [];
            if (!completedLayerBatches.includes(batchIndex)) {
                completedLayerBatches.push(batchIndex);
            }
            
            await updateMasterTaskProgress(env, masterTaskId, {
                processedLayers,
                completedLayerBatches,
            });
            ...
            lastError = err;
            console.error(`Layer attempt ${attempt + 1} failed for ${digest}:`, err);
            if (attempt < MAX_RETRIES - 1) {
                await new Promise(resolve => setTimeout(resolve, RETRY_DELAY * (attempt + 1)));
            }
        }
    ...
    const masterKey = `master:${masterTaskId}`;
    const master = await env.B2_KV.get(masterKey, 'json') || {};
    const failedLayers = master.failedLayers || [];
    failedLayers.push({ digest, tag, error: lastError.message });
    ...
    await checkAllBatchesCompleted(env, masterTaskId);
    console.error(`Layer ${digest} permanently failed after ${MAX_RETRIES} attempts.`);
}
