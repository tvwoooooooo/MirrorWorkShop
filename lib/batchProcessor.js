 ...
 * Docker 镜像备份处理
 */
export async function processDockerImage(task, env) {
    const { bucketId, namespace, repo, tag, digest, size } = task;
    const { client, bucket } = await getB2Client(bucketId, env);
    ...
    // 获取用户提供的 Docker 令牌（如果有）
    const tokens = await env.DB.prepare("SELECT token FROM tokens WHERE type = ? ORDER BY id").bind('docker').all();
    const userToken = tokens.results.length > 0 ? tokens.results[0].token : null;

    // 获取 registry token
    async function getDockerToken() {
    ...
    // 上传 manifest
    const manifestKey = `${namespace}/${repo}/${tag}/manifest.json`;
    const manifestBody = JSON.stringify(manifest, null, 2);
    await uploadFile(client, bucket, manifestKey, new Blob([manifestBody]).stream(), manifestBody.length);

    // 下载并上传 config
    const configDigest = manifest.config.digest;
    const configSize = manifest.config.size;
    const configRes = await fetch(`${DOCKER_REGISTRY}/v2/${namespace}/${repo}/blobs/${configDigest}`, {
        headers: { 'Authorization': `Bearer ${token}` }
    });
    if (!configRes.ok) throw new Error(`Config download failed: ${configRes.status}`);
    const configKey = `${namespace}/${repo}/${tag}/config.json`;
    await uploadFile(client, bucket, configKey, configRes.body, configSize);
    ...
    for (let i = 0; i < layers.length; i++) {
        ...
        const layerRes = await fetch(`${DOCKER_REGISTRY}/v2/${namespace}/${repo}/blobs/${layerDigest}`, {
            headers: { 'Authorization': `Bearer ${token}` }
        });
        if (!layerRes.ok) throw new Error(`Layer download failed: ${layerRes.status}`);
        const layerKey = `${namespace}/${repo}/${tag}/layers/${layerDigest.replace(':', '_')}.tar`;
        await uploadFile(client, bucket, layerKey, layerRes.body, layerSize);
    }
