...
import { getDockerLayer } from './docker.js';
...
export async function processDockerLayer(layerTask, env) {
    const { masterTaskId, bucketId, owner, repo, asset, batchIndex, totalBatches } = layerTask;
    ...
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
        try {
            // 从 Docker Registry 获取层数据流
            const image = `${owner}/${repo}`;
            const layerRes = await getDockerLayer(image, asset.digest, env);
            if (!layerRes.ok) throw new Error(`Layer download failed: ${layerRes.status}`);

            // 获取大小（从 asset 中已有 size）
            const length = asset.size;

            // 构建 B2 路径：例如 owner/repo/layers/digest.tar.gz
            const b2Path = `${owner}/${repo}/layers/${asset.name}`;

            // 上传（使用流式上传，如果层很大可能需分片，但 asset 已预知大小，若大于阈值则分片）
            if (length < MULTIPART_THRESHOLD) {
                await uploadFile(client, bucket, b2Path, layerRes.body, length);
            } else {
                // 启用分片上传（假设 Docker Registry 支持 Range 请求）
                await multipartUpload(client, bucket, b2Path, length, async (start, end) => {
                    const rangeRes = await getDockerLayer(image, asset.digest, env, { headers: { 'Range': `bytes=${start}-${end}` } });
                    if (!rangeRes.ok) throw new Error(`Range request failed: ${rangeRes.status}`);
                    return rangeRes.body;
                });
            }
            ...
            lastError = err;
            console.error(`Docker layer attempt ${attempt + 1} failed:`, err);
            if (attempt < MAX_RETRIES - 1) {
                await new Promise(resolve => setTimeout(resolve, RETRY_DELAY * (attempt + 1)));
            }
        }
    ...
    const master = await getMasterTask(env, masterTaskId) || {};
    const failedAssets = master.failedAssets || [];
    failedAssets.push({ name: asset.name, digest: asset.digest, error: lastError.message });
    ...
    await checkAllBatchesCompleted(env, masterTaskId);
    console.error(`Docker layer ${asset.name} permanently failed after ${MAX_RETRIES} attempts.`);
}