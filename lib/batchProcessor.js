...
import { getDockerRegistryToken, getDockerImageManifest } from './docker.js';
 ...
 * 如果文件为空，则直接普通上传空内容。
 */
async function multipartUploadUnknownSize(b2Client, bucket, key, url, headers = {}) {
    const response = await fetchWithRetry(url, {
        headers: { 'User-Agent': 'B2-Mirror-Worker', ...headers }
    });
...
export async function processDockerLayer(task, env) {
    const { masterTaskId, bucketId, imageName, digest, batchIndex } = task.asset;

    const { client, bucket } = await getB2Client(bucketId, env);

    try {
        const token = await getDockerRegistryToken(imageName, env, []);
        if (!token) {
            throw new Error(`Failed to get registry token for ${imageName}`);
        }

        const layerUrl = `https://registry-1.docker.io/v2/${imageName}/blobs/${digest}`;
        const b2Path = `docker/layers/${digest}`;
        
        await multipartUploadUnknownSize(client, bucket, b2Path, layerUrl, {
        ...
        console.error(`Docker layer ${digest} failed:`, err);
        const master = await getMasterTask(env, masterTaskId) || {};
        const failedAssets = master.failedAssets || [];
        failedAssets.push({ name: digest, error: err.message });
...
export async function processDockerManifest(task, env) {
    const { masterTaskId, bucketId, imageName, tag, batchIndex } = task.asset;
    const { client, bucket } = await getB2Client(bucketId, env);

    try {
        const { manifest } = await getDockerImageManifest(imageName, tag, env);
        if (!manifest) {
            throw new Error(`Failed to get manifest for ${imageName}:${tag}`);
        }
        const manifestBlob = new Blob([JSON.stringify(manifest)], { type: 'application/vnd.docker.distribution.manifest.v2+json' });

        const b2Path = `docker/manifests/${imageName}/${tag}.json`;
        await uploadFile(client, bucket, b2Path, manifestBlob.stream(), manifestBlob.size);
        ...
        console.error(`Docker manifest for ${imageName}:${tag} failed:`, err);
        const master = await getMasterTask(env, masterTaskId) || {};
        const failedAssets = master.failedAssets || [];
        failedAssets.push({ name: `${imageName}:${tag}`, error: err.message });
