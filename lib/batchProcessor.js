 ...
 * @param {string} bucketId - 桶 ID
 * @param {string} metaPath - 元数据文件路径（如 owner/repo/date/_manifest.json）
 * @param {Object} metadata - 要存储的元数据对象
 */
export async function uploadMetadata(env, bucketId, metaPath, metadata) {
    const { client, bucket } = await getB2Client(bucketId, env);
    const metadataString = JSON.stringify(metadata, null, 2);
    await uploadFile(client, bucket, metaPath, metadataString, metadataString.length);
    console.log(`[uploadMetadata] Uploaded metadata to ${metaPath}`);
}
...
async function checkAllBatchesCompleted(env, masterTaskId) {
    ...
    if (allFileBatchesDone && allAssetBatchesDone) {
        ...
        let metaPath = null;
        // 如果是 GitHub 项目，先生成并上传元数据文件
        if (master.owner !== 'docker') {
            // 使用任务创建日期作为备份日期（避免同一天多次备份冲突，可考虑使用时间戳）
            const backupDate = new Date(master.createdAt).toISOString().split('T')[0];
            const backupTime = Date.now(); // 或使用 master.createdAt 作为时间戳
            // 使用时间戳作为子目录或文件名的一部分，避免覆盖
            const datePath = `${backupDate}/${backupTime}`;
            metaPath = `${master.owner}/${master.repo}/${datePath}/_manifest.json`;
            const metadata = {
                backupId: master.taskId,
                backupTime: master.createdAt,
                date: backupDate,
                files: master.filePaths || [],
                failedFiles: master.failedFiles || [],
                releases: [] // 可后续从 master 中获取 releases
            };
            try {
                await uploadMetadata(env, master.bucketId, metaPath, metadata);
                console.log(`[checkAllBatchesCompleted] Uploaded metadata for ${master.owner}/${master.repo} to ${metaPath}`);
            } catch (e) {
                console.error(`Failed to upload metadata for GitHub project ${master.owner}/${master.repo}:`, e);
                // 即使元数据上传失败，任务仍可完成，但需要记录失败？
                // 我们将继续完成，但不传递 metaPath
            }
        }

        // 调用 completeMasterTask，并传入 metaPath（如果有）
        // 注意：completeMasterTask 需要修改以接受 metaPath 参数
        await completeMasterTask(env, masterTaskId, finalStatus, master.failedFiles, master.failedAssets, metaPath);
    }
}
