...
export async function processBatch(batchTask, env) {
    ...
    const master = await getMasterTask(env, masterTaskId) || {};
    const metadata = master.metadata || { files: [], assets: [], fileSizes: {}, assetSizes: {} };
    
    // 确保子对象存在
    metadata.fileSizes = metadata.fileSizes || {};
    metadata.assetSizes = metadata.assetSizes || {};
    metadata.fileDates = metadata.fileDates || {};
