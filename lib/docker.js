// lib/docker.js
const DOCKER_HUB_SEARCH_API = 'https://hub.docker.com/api/content/v1/products/search';
...
export async function searchDockerHub(query, page = 1, perPage = 30, env) {
    ...
    const url = `${DOCKER_HUB_SEARCH_API}?q=${encodeURIComponent(query)}&page=${page}&page_size=${perPage}&type=image`;

    try {
        const response = await fetchWithTimeout(url, { headers });

        if (!response.ok) {
            // 对于429或其他错误，直接抛出，由外部调用处理
        ...
        const items = data.summaries.map(item => ({
            name: item.slug,
            ...
            homepage: `https://hub.docker.com/r/${item.slug}`,
            type: 'docker',
            owner: item.slug.split('/')[0],
            repo: item.slug.split('/')[1] || item.slug,
        }));
