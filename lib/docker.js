...
/**
 * 获取 Docker registry 的临时 token（使用用户提供的 PAT 进行 Basic 认证）
...
async function getRegistryToken(image, pat) {
    ...
    const headers = {};
    if (pat) {
        // 使用 Basic 认证，username 可以是任意字符串，密码为 PAT
        const credentials = btoa(`docker:${pat}`);
    ...
    for (let attempt = 0; attempt <= (hasTokens ? tokens.length : 0); attempt++) {
        ...
        try {
            ...
            if (manifest.mediaType === 'application/vnd.docker.distribution.manifest.list.v2+json') {
                ...
                // 递归获取具体的 manifest（注意这里使用 digest 而不是 tag）
