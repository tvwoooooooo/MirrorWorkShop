...
export async function searchDockerHub(query, page = 1, perPage = 30, env) {
    ...
    try {
        ...
        if (!Array.isArray(data)) {
            logMessages.push("[ERROR] Parsed data is not an array. Aborting.");
        ...
        await Promise.all(
            items.map(async (item, idx) => {
                const { hasTags, logs } = await checkDockerHasTags(item.name, env);
                items[idx].has_releases = hasTags;
                logMessages.push(...logs);
            })
        );

        logMessages.push("--- searchDockerHub finished successfully. ---");
        return { items: items, total: items.length, logs: logMessages };
