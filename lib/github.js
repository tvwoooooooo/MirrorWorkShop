...
// 获取下一个令牌索引（基于 D1 的全局计数器）
async function getNextTokenIndex(env, tokenCount) {
    ...
    for (let attempt = 0; attempt <= retries + (hasTokens ? tokens.length : 0); attempt++) {
        try {
            ...
            if (attempt === 0) {
                ...
                const tokenIndex = await getNextTokenIndex(env, tokens.length);
                if (tokenIndex >= 0) {
