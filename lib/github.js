...
export async function fetchWithRetry(url, options, retries = 3, token = null) {
...
export async function getNextGithubToken(env) {
    const tokens = await env.B2_KV.get('github_tokens', 'json') || [];
    if (tokens.length === 0) return null;
    lastTokenIndex = (lastTokenIndex + 1) % tokens.length;
    const token = tokens[lastTokenIndex].token;
    tokens[lastTokenIndex].usageCount = (tokens[lastTokenIndex].usageCount || 0) + 1;
    await env.B2_KV.put('github_tokens', JSON.stringify(tokens));
    return token;
}

export async function githubFetch(url, options, env) {
    try {
        return await fetchWithRetry(url, options, 1, null);
    } catch (e) {
        if (e.message.includes('403')) {
            const token = await getNextGithubToken(env);
            if (token) {
                return await fetchWithRetry(url, options, 3, token);
            }
        }
        throw e;
    }
