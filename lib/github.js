...
// 轮次重置阈值，当轮次达到此值时重置为 1（可调整）
const ROUND_RESET_THRESHOLD = 1000000;
...
async function getCurrentRound(env) {
    ...
    if (!result) {
        ...
        // 检查是否需要重置轮次
        if (round >= ROUND_RESET_THRESHOLD) {
            // 在事务中重置所有令牌的 round_used 并将轮次设为 1
            await env.DB.batch([
                env.DB.prepare("UPDATE tokens SET round_used = 0 WHERE type = ?").bind('github'),
                env.DB.prepare("UPDATE token_round SET round = 1 WHERE type = ?").bind('github')
            ]);
            round = 1;
            // 使令牌缓存失效，因为 round_used 已改变
            cachedTokens = null;
...
// 递增轮次（更新缓存），并在递增后检查是否需要重置（但重置逻辑已在 getCurrentRound 中处理）
        ...
        try {
            ...
            if (attempt === 0) {
                ...
                if (!availableToken) {
                    ...
                    currentRound = await getCurrentRound(env); // 获取新轮次（可能触发重置）
                    // 重新获取令牌列表（因为 round_used 可能已更新）
