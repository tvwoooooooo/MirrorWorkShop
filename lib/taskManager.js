...
/**
 * 创建主任务，并发送文件批次到队列
 * 对于 GitHub 项目，metadata 中存储 { files: filePaths, assets: assets, fileSizes: {} }
 */
export async function createMasterTask(env, taskId, owner, repo, bucketId, filePaths, assets = []) {
    ...
    // 构建 metadata：存储完整的文件列表、资产列表和文件大小信息
    ...
    return {
        ...
        processedAssets: 0,
        failedAssets: [],
        metadata: { files: filePaths, assets: assets, fileSizes: {}, assetSizes: {} },
        status: 'processing',
        createdAt: Date.now()
    };
}
...
export async function saveProjectToDb(env, task, tags = null) {
    ...
    let name, homepage, metaData, hasReleases;
    ...
    if (type === 'github') {
        ...
        // 判断是否有 releases
        hasReleases = assetList.length > 0;
        ...
        hasReleases = tagList.length > 0; // Docker 的 tags 可视为 releases
        metaData = {
            backupId: taskId,
            date: now,
            type: 'docker',
    ...
    // 添加新版本信息（包含 has_releases 标志）
    const newVersion = {
        backupId: taskId,
        date: now,
        metaPath: metaPath,
        bucketId: bucketId,
        has_releases: hasReleases  // 新增标志
    };
