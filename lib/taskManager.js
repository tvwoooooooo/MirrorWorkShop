...
export async function createMasterTask(env, taskId, owner, repo, bucketId, filePaths, assets = []) {
    console.log(`[taskManager] Creating master task ${taskId} for ${owner}/${repo}`);
    ...
    try {
        ...
        console.log(`[taskManager] Inserting into master_tasks for ${taskId}`);
        ...
        console.log(`[taskManager] Successfully inserted into master_tasks for ${taskId}`);
        
        console.log(`[taskManager] Inserting into active_tasks for ${taskId}`);
        await env.DB.prepare(`
            INSERT OR REPLACE INTO active_tasks (task_id, name, status) VALUES (?, ?, ?)
        `).bind(taskId, `${owner}/${repo}`, 'processing').run();
        console.log(`[taskManager] Successfully inserted into active_tasks for ${taskId}`);

        console.log(`[taskManager] Sending ${fileBatches.length} file batches to queue for ${taskId}`);
        ...
        console.log(`[taskManager] Sending ${assets.length} asset batches to queue for ${taskId}`);
        ...
        console.log(`[taskManager] All tasks sent to queue for ${taskId}`);

    } catch (e) {
        console.error(`[taskManager] CRITICAL: Failed to create master task ${taskId}. Error: ${e.message}`, e.stack);
