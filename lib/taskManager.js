...
export async function saveProjectToDb(env, task, tags = null) {
    ...
    const { owner, repo, status, metadata } = task;
    ...
    if (type === 'github') {
        ...
        versions = [{
            date: now,
            files: fileList,
            releases: assetList.map(a => ({ name: a.name, url: a.url, size: a.size })) // 简化资产信息
        }];
    } else {
        name = repo; // 对于 Docker，repo 就是完整镜像名，如 xhofe/alist
        homepage = `https://hub.docker.com/r/${repo}`;
        // 如果传入 tags，则使用；否则尝试从 metadata 中获取（如果 docker-master 任务存入了 metadata）
    ...
    console.log(`[saveProjectToDb] Saved ${type} project: ${name}`);
}