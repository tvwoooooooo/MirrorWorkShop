...
/**
 * 创建主任务，并发送文件批次到队列
 * 对于 GitHub 项目，metadata 中存储 { files: filePaths, assets: assets, fileSizes: {}, assetSizes: {}, fileDates: {}, description }
 */
export async function createMasterTask(env, taskId, owner, repo, bucketId, filePaths, assets = [], description = '') {
    ...
    // 构建 metadata：存储完整的文件列表、资产列表和大小、日期信息
    ...
    return {
        ...
        processedAssets: 0,
        failedAssets: [],
        metadata: { files: filePaths, assets: assets, fileSizes: {}, assetSizes: {}, fileDates: {}, description },
        status: 'processing',
        createdAt: Date.now()
    };
}
...
export async function saveProjectToDb(env, task, tags = null) {
    ...
    if (type === 'github') {
        ...
        const fileDates = metadata?.fileDates || {};
        ...
        const filesWithInfo = fileList.map(path => ({
            ...
            lastModified: fileDates[path] || null
        }));
