...
/**
 * 创建主任务，并发送文件批次到队列
 * 对于 GitHub 项目，metadata 中存储 { files: filePaths, assets: assets, fileSizes: {}, assetSizes: {}, fileDates: {}, description: description }
...
export async function createMasterTask(env, taskId, owner, repo, bucketId, filePaths, assets = [], description = '') {
    ...
    const metadata = JSON.stringify({
        ...
        fileDates: {}        // 键为文件路径，值为最后修改日期（来自 GitHub）
    });
...
export async function saveProjectToDb(env, task, tags = null) {
    ...
    if (type === 'github') {
        ...
        const fileDates = metadata?.fileDates || {}; // 新增
        ...
        const filesWithInfo = fileList.map(path => ({
            ...
            lastModified: fileDates[path] || null // 添加最后修改日期
        }));
