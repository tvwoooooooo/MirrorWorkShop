...
/**
 * 创建主任务，并发送文件批次到队列
 * 对于 GitHub 项目，metadata 中存储 { files: filePaths, assets: assets, description: description }
 */
export async function createMasterTask(env, taskId, owner, repo, bucketId, filePaths, assets = [], description = '') {
    ...
    return {
        ...
        processedAssets: 0,
        failedAssets: [],
        metadata: { files: filePaths, assets: assets, description: description, fileSizes: {}, assetSizes: {} },
        status: 'processing',
        createdAt: Date.now()
    };
}
...
export async function updateMasterTaskProgress(env, taskId, updates) {
    ...
    const metadata = JSON.stringify(master.metadata);
...
export async function saveProjectToDb(env, task, tags = null) {
    ...
    if (type === 'github') {
        ...
        const releasesWithInfo = assetList.map(asset => ({
            name: asset.name,
            url: asset.url,
            size: asset.size || assetSizes[asset.url] || 0,
            key: `${owner}/${repo}/releases/${asset.name}` // 实际存储路径
        }));
    ...
    // 插入或替换项目记录（包含描述）
    await env.DB.prepare(`
        INSERT OR REPLACE INTO projects (type, name, homepage, last_update, versions, description)
        VALUES (?, ?, ?, ?, ?, ?)
    `).bind(type, name, homepage, now, JSON.stringify(versions), description).run();
