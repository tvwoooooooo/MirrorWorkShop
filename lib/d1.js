...
/**
 * 确保 master_tasks 表存在，并添加 metadata 列（如果不存在）
 */
export async function ensureMasterTasksTable(env) {
    ...
    // 尝试添加 metadata 列（如果表已存在但缺少该列）
    try {
        await env.DB.prepare(`ALTER TABLE master_tasks ADD COLUMN metadata TEXT;`).run();
    } catch (e) {
        // 如果列已存在，SQLite 会抛出错误，忽略即可
    }
}
...
/**
 * 确保 projects 表存在，并添加 description 和 has_releases 列（如果不存在）
 */
export async function ensureProjectsTable(env) {
    ...
    // 尝试添加 description 列（如果表已存在但缺少该列）
    try {
    ...
    // 尝试添加 has_releases 列
    try {
        await env.DB.prepare(`ALTER TABLE projects ADD COLUMN has_releases INTEGER DEFAULT 0;`).run();
    } catch (e) {
        // 忽略
    }
}
