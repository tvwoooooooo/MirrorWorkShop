...
(async function() {
    ...
    let currentReleases = [];
    let selectedReleaseAssets = {}; // 格式: { "v1.0.0": ["asset1.zip", "asset2.tar.gz"] }
        ...
        verifyBucketBtn.addEventListener('click', async () => {
            ...
            try {
                const res = await fetch(apiBase + '/verify-bucket', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ keyID, applicationKey: appKey, bucketName: bktName, endpoint })
                });
    ...
    if (closeSelectBucketModal) {
        closeSelectBucketModal.addEventListener('click', () => { selectBucketModal.style.display = 'none'; });
    }
    if (selectBucketModal) {
        selectBucketModal.addEventListener('click', (e) => { if (e.target === selectBucketModal) selectBucketModal.style.display = 'none'; });
    }
    ...
    if (confirmSelectBucketBtn) {
        confirmSelectBucketBtn.addEventListener('click', async () => {
            ...
            currentBucketId = selectedCard.dataset.bucketId;
            selectBucketModal.style.display = 'none';
            await openBackupContentModal(currentProjectToBackup);
        });
    }
    ...
    async function openBackupContentModal(project) {
        const modal = safeGet('backupContentModal');
        const releasesSelector = safeGet('releasesSelector');
        const backupReleasesCheck = safeGet('backupReleases');
        const backupCodeFilesCheck = safeGet('backupCodeFiles');
        const releaseVersionSelector = safeGet('backupReleaseVersionSelector');
        const selectedReleaseVersionSpan = safeGet('selectedReleaseVersion');
        const releaseVersionDropdown = safeGet('backupReleaseVersionDropdown');
        const assetsList = safeGet('backupAssetsList');
        const selectAllAssetsCheck = safeGet('selectAllAssets');

        // 重置
        backupReleasesCheck.checked = false;
        releasesSelector.style.display = 'none';
        backupCodeFilesCheck.checked = true;
        selectedReleaseAssets = {};

        // 获取项目的 releases
        if (project.type === 'github') {
            try {
                const url = \`https://api.github.com/repos/\${project.owner}/\${project.repo}/releases?per_page=100\`;
                const res = await fetch(url, {
                    headers: { 'Accept': 'application/vnd.github.v3+json', 'User-Agent': 'B2-Mirror-Worker' }
                });
                if (res.ok) {
                    const data = await res.json();
                    currentReleases = data.map(r => ({
                        tag: r.tag_name,
                        date: r.published_at?.split('T')[0] || r.created_at?.split('T')[0],
                        assets: r.assets.map(a => ({
                            name: a.name,
                    ...
                    if (currentReleases.length > 0) {
                        renderReleaseVersionDropdown(currentReleases, 0);
                    } else {
                        currentReleases = [];
                    }
                } else {
                ...
                console.error('获取 releases 失败', e);
                currentReleases = [];
            }
        }

        backupReleasesCheck.onchange = () => {
            if (backupReleasesCheck.checked) {
                releasesSelector.style.display = 'block';
                if (currentReleases.length > 0) {
                    renderAssetsForRelease(0);
                } else {
                    assetsList.innerHTML = '<div class="empty-state">该项目暂无 Releases</div>';
                }
            } else {
                releasesSelector.style.display = 'none';
            }
        ...
        selectAllAssetsCheck.onchange = () => {
            const checkboxes = assetsList.querySelectorAll('.asset-checkbox');
            checkboxes.forEach(cb => {
                cb.checked = selectAllAssetsCheck.checked;
    ...
    function renderReleaseVersionDropdown(releases, activeIndex) {
        const dropdown = safeGet('backupReleaseVersionDropdown');
        const selectedSpan = safeGet('selectedReleaseVersion');
        if (!dropdown || !selectedSpan) return;
        dropdown.innerHTML = releases.map((r, idx) => \`
            <div class="version-item-sm \${idx === activeIndex ? 'current' : ''}" data-release-index="\${idx}">\${r.date} (\${r.tag})</div>
        \`).join('');
        selectedSpan.innerText = releases[activeIndex].date + ' (' + releases[activeIndex].tag + ')';
        
        dropdown.querySelectorAll('.version-item-sm').forEach(item => {
            item.addEventListener('click', (e) => {
                e.stopPropagation();
                const idx = parseInt(item.dataset.releaseIndex);
                renderReleaseVersionDropdown(releases, idx);
                renderAssetsForRelease(idx);
                dropdown.classList.remove('show');
            });
        });
        
        const selector = safeGet('backupReleaseVersionSelector');
        selector.onclick = (e) => {
            e.stopPropagation();
    ...
    function renderAssetsForRelease(releaseIdx) {
        const assetsList = safeGet('backupAssetsList');
        const release = currentReleases[releaseIdx];
        if (!release) return;
        if (release.assets.length === 0) {
            assetsList.innerHTML = '<div class="empty-state">该版本无 assets 文件</div>';
            return;
        }
        let html = '';
        release.assets.forEach((asset, assetIdx) => {
            const checked = selectedReleaseAssets[release.tag]?.includes(asset.name) ? 'checked' : '';
            html += \`
                <div class="asset-row" style="padding: 0.5rem; border-bottom: 1px solid #e2e8f0;">
                    <label style="display: flex; align-items: center; gap: 0.5rem;">
                        <input type="checkbox" class="asset-checkbox" data-release-tag="\${release.tag}" data-asset-name="\${asset.name}" \${checked}>
                        <span class="asset-name">\${asset.name}</span>
                        <span style="color:#64748b; font-size:0.8rem;">\${(asset.size/1024).toFixed(2)} KB</span>
                    </label>
                </div>
            \`;
        });
        ...
        assetsList.querySelectorAll('.asset-checkbox').forEach(cb => {
            cb.addEventListener('change', (e) => {
                const tag = cb.dataset.releaseTag;
                const assetName = cb.dataset.assetName;
                if (!selectedReleaseAssets[tag]) {
                    selectedReleaseAssets[tag] = [];
                }
                if (cb.checked) {
                    if (!selectedReleaseAssets[tag].includes(assetName)) {
                        selectedReleaseAssets[tag].push(assetName);
                    }
                } else {
                    selectedReleaseAssets[tag] = selectedReleaseAssets[tag].filter(name => name !== assetName);
                    if (selectedReleaseAssets[tag].length === 0) {
                        delete selectedReleaseAssets[tag];
                    }
                }
            });
        });
    ...
    const confirmBackupContentBtn = safeGet('confirmBackupContent');
    if (confirmBackupContentBtn) {
        confirmBackupContentBtn.addEventListener('click', async () => {
            const backupCodeFiles = safeGet('backupCodeFiles').checked;
            const backupReleases = safeGet('backupReleases').checked;
            const releasesToBackup = [];

            if (backupReleases) {
                for (const tag in selectedReleaseAssets) {
                    releasesToBackup.push({
                        tag,
                        assets: selectedReleaseAssets[tag]
                    });
                }
            }
            ...
            const backupOptions = {
                codeFiles: backupCodeFiles,
                releases: releasesToBackup
            };
            ...
            if (result.success) {
                ...
                safeGet('backupContentModal').style.display = 'none';
    ...
    const cancelBackupContent = safeGet('cancelBackupContent');
    if (cancelBackupContent) {
        cancelBackupContent.addEventListener('click', () => {
            safeGet('backupContentModal').style.display = 'none';
        });
    }

    const closeBackupContentModal = safeGet('closeBackupContentModal');
    if (closeBackupContentModal) {
        closeBackupContentModal.addEventListener('click', () => {
            safeGet('backupContentModal').style.display = 'none';
        });
    }
