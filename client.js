...
(async function() {
    ...
    if (confirmSelectBucketBtn) {
        confirmSelectBucketBtn.addEventListener('click', async () => {
            ...
            const backupOptions = {
                backupCode,
                releases: selectedReleaseAssets
            };
    ...
    async function openReleasesSelectPopup(project) {
        currentProjectReleases = project;
        const popup = safeGet('releasesSelectPopup');
        const projectNameEl = safeGet('releasesSelectProjectName');
        const versionSelector = safeGet('releasesSelectVersionSelector');
        const selectedVersionEl = safeGet('releasesSelectSelectedVersion');
        const versionDropdown = safeGet('releasesSelectVersionDropdown');
        const releasesList = safeGet('releasesSelectList');
        const selectAllCheck = safeGet('selectAllReleasesCheckbox');
        ...
        try {
            if (project.type === 'github') {
                const url = \`https://api.github.com/repos/\${project.owner}/\${project.repo}/releases\`;
                const res = await fetch(url, {
                    headers: { 'Accept': 'application/vnd.github.v3+json', 'User-Agent': 'B2-Mirror-Worker' }
                });
                if (res.ok) {
        ...
        if (releases.length === 0) {
            releasesList.innerHTML = '<div style="padding: 2rem; text-align: center;">该项目暂无 Releases</div>';
        ...
        const projectKey = \`\${project.type}:\${project.name}\`;
        const savedSelections = selectedReleases.get(projectKey) || [];
        
        function renderVersionDropdown() {
            let html = '';
            releases.forEach((r, idx) => {
                html += \`<div class="version-item-sm" data-version-index="\${idx}">\${r.date} - \${r.tag}</div>\`;
            });
            versionDropdown.innerHTML = html;
        }
        
        function renderReleasesList(versionIdx) {
            const version = releases[versionIdx];
            let listHtml = '';
            version.assets.forEach(asset => {
                const isSelected = savedSelections.some(s => s.tag === version.tag && s.assetName === asset.name);
                listHtml += \`
                    <div class="asset-row">
                        <label class="checkbox-item" style="flex:1;">
                            <input type="checkbox" class="release-asset-checkbox" data-tag="\${version.tag}" data-asset="\${asset.name}" \${isSelected ? 'checked' : ''}>
            ...
            releasesList.innerHTML = listHtml || '<div class="asset-row">该版本无资产文件</div>';
        }
        
        renderVersionDropdown();
        renderReleasesList(currentVersionIdx);
        selectedVersionEl.innerText = releases[currentVersionIdx].date;
        ...
        versionDropdown.querySelectorAll('.version-item-sm').forEach(item => {
            item.addEventListener('click', (e) => {
                e.stopPropagation();
                const idx = parseInt(item.dataset.versionIndex);
                if (idx !== currentVersionIdx) {
                    currentVersionIdx = idx;
                    selectedVersionEl.innerText = releases[idx].date;
                    renderReleasesList(idx);
                }
        ...
        selectAllCheck.addEventListener('change', (e) => {
            const checkboxes = document.querySelectorAll('.release-asset-checkbox');
    ...
    function closeReleasesSelectPopup(save = true) {
        const popup = safeGet('releasesSelectPopup');
        if (!popup) return;
        if (save && currentProjectReleases) {
            const projectKey = \`\${currentProjectReleases.type}:\${currentProjectReleases.name}\`;
            const selected = [];
            document.querySelectorAll('.release-asset-checkbox:checked').forEach(cb => {
                selected.push({
                    tag: cb.dataset.tag,
                    assetName: cb.dataset.asset
                });
            });
    ...
    const releasesSelectPopupClose = safeGet('releasesSelectPopupClose');
    if (releasesSelectPopupClose) {
        releasesSelectPopupClose.addEventListener('click', () => closeReleasesSelectPopup(true));
    }

    const confirmReleasesSelectBtn = safeGet('confirmReleasesSelectBtn');
    if (confirmReleasesSelectBtn) {
        confirmReleasesSelectBtn.addEventListener('click', () => closeReleasesSelectPopup(true));
    }

    const releasesSelectPopup = safeGet('releasesSelectPopup');
    if (releasesSelectPopup) {
        releasesSelectPopup.addEventListener('click', (e) => {
            if (e.target === releasesSelectPopup) closeReleasesSelectPopup(false);
        });
    }
