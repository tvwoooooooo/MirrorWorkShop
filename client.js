...
(async function() {
    ...
    // 新增：Docker 备份相关变量
    let dockerBackupData = null;          // { namespace, repo, name }
    let dockerTags = [];                  // 标签列表 [{ name, digest, size, lastUpdate }]
    let selectedTags = new Set();         // 选中的标签 name
    ...
    async function loadOfficialResults(query, type, page) {
        ...
        try {
            ...
            newItems.forEach(item => {
                ...
                const releaseButton = isGitHub && item.has_releases ? \`<button class="btn-icon btn-release" data-project='\${JSON.stringify(item)}'><i class="fas fa-tag"></i> Releases</button>\` : '';
                const itemHtml = \`
    ...
    async function loadAdminResults(query, type, page) {
        ...
        try {
            ...
            newItems.forEach(item => {
                ...
                const itemHtml = \`
                    <div class="search-result-item">
                        ...
                        <div>
                            <button class="save-btn backup-btn" data-name="\${item.name}" data-type="\${item.type}" data-owner="\${item.owner}" data-repo="\${item.repo}" data-namespace="\${item.namespace}">完整备份</button>
                        </div>
                    </div>\`;
            ...
            // 绑定完整备份按钮事件（新流程，区分 GitHub 和 Docker）
            document.querySelectorAll('.backup-btn').forEach(btn => {
                btn.addEventListener('click', (e) => {
                    ...
                    if (type === 'github') {
                        ...
                        const repo = e.target.dataset.repo;
                        openDockerBackupModal({ name, namespace, repo });
                    }
                });
            });
    ...
    const githubFileSection = safeGet('githubFileSection');
    const dockerTagSection = safeGet('dockerTagSection');

    async function openBackupContentModal(project) {
        backupProjectData = project;
        dockerBackupData = null; // 清除 Docker 数据
        ...
        backupTypeIcon.className = 'fab fa-github';
        // 显示 GitHub 区域，隐藏 Docker 区域
        if (githubFileSection) githubFileSection.style.display = 'block';
        if (dockerTagSection) dockerTagSection.style.display = 'none';
        ...
        backupSaveBtn.addEventListener('click', async () => {
            ...
            if (backupProjectData) {
                ...
                const tags = Array.from(selectedTags).map(tagName => {
                    const tag = dockerTags.find(t => t.name === tagName);
                    return { name: tag.name, digest: tag.digest, size: tag.size };
                });

                const payload = {
                    namespace: dockerBackupData.namespace,
                    repo: dockerBackupData.repo,
                    bucketId,
                    tags
                };

                try {
                    const res = await fetch(apiBase + '/docker/backup', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify(payload)
                    });
                    const result = await res.json();
                    if (result.success) {
                        alert(\`Docker 镜像备份任务已提交，任务ID: \${result.taskId}\`);
                        pollTaskStatus(result.taskId);
                        backupModal.style.display = 'none';
                    } else {
                        alert('保存失败：' + (result.error || '未知错误'));
    ...
    async function openDockerBackupModal(project) {
        dockerBackupData = project;
        backupProjectData = null; // 清除 GitHub 数据
        backupModal.style.display = 'flex';
        backupStep1.classList.remove('hide');
        backupStep2.classList.add('hide');
        backupPrevBtn.style.display = 'none';
        ...
        backupSaveBtn.style.display = 'none';
        backupProjectName.innerText = project.name;
        backupTypeIcon.className = 'fab fa-docker';
        // 隐藏 GitHub 区域，显示 Docker 区域
        if (githubFileSection) githubFileSection.style.display = 'none';
        if (dockerTagSection) dockerTagSection.style.display = 'block';

        tagsContainer.innerHTML = '<div class="loading-indicator">加载标签中...</div>';
        selectedTags.clear();
        if (selectAllTags) selectAllTags.checked = false;
        if (selectedTagsCount) selectedTagsCount.innerText = '0 个标签';

        try {
            const res = await fetch(\`/api/docker/tags?namespace=\${project.namespace}&repo=\${project.repo}\`);
            if (!res.ok) throw new Error('获取标签失败');
            dockerTags = await res.json();
            renderTags();
        } catch (e) {
    ...
    function renderTags() {
        if (!dockerTags || dockerTags.length === 0) {
            tagsContainer.innerHTML = '<div class="empty-state">无标签</div>';
            return;
        }
        let html = '';
        dockerTags.forEach(tag => {
            const size = tag.size ? (tag.size / (1024*1024)).toFixed(2) + ' MB' : '未知';
            html += \`
                <div class="tag-item">
                    <input type="checkbox" class="tag-checkbox" data-tag-name="\${tag.name}">
                    <span class="tag-name">\${tag.name}</span>
                    <span class="tag-size">\${size}</span>
                    <span class="tag-date">\${tag.lastUpdate ? tag.lastUpdate.split('T')[0] : ''}</span>
                </div>
            \`;
        });
    ...
    function updateSelectedTags() {
        ...
        if (selectedTagsCount) selectedTagsCount.innerText = \`\${count} 个标签\`;

        if (selectAllTags) {
            const total = document.querySelectorAll('.tag-checkbox').length;
            selectAllTags.checked = count === total;
            selectAllTags.indeterminate = count > 0 && count < total;
        }
    }
